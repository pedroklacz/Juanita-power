...
#keys
with open('src/day/keys.json') as json_file:
...
while True:
    ...
    if status:
        if 'estas ahi' in rec:
            ...
            nt(f"Vuelve a intentarlo, no reconozco: {rec}")
        
        attemts = 0
    else:
        attemts += 1

print(f"{red_color} PROGRAMA FINALIZADO CON UNA DURACIÓN DE: { int(time.time() - start_time) } SEGUNDOS {normal_color}")